    ...
    <div className="flex min-h-screen">
      ...
      <div className="hidden w-1/2 flex-col justify-between bg-primary p-12 lg:flex">
        <div className="flex items-center gap-3">
          ...
          <span className="text-2xl font-bold text-primary-foreground">Fungura</span>
        </div>
      ...
      <div className="flex flex-1 items-center justify-center bg-background p-8">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "signup" && (
              <div>
                <label className="mb-1 block text-xs font-medium text-foreground">{t("full_name")}</label>
                <div className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2.5">
                  <User className="h-4 w-4 text-muted-foreground" />
                  <input placeholder={t("enter_name")} className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
                </div>
              </div>
            )}

            <div>
              <label className="mb-1 block text-xs font-medium text-foreground">{t("email")}</label>
              <div className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2.5">
                <Mail className="h-4 w-4 text-muted-foreground" />
                <input type="email" placeholder={t("enter_email")} className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
              </div>
            </div>

            <div>
              <label className="mb-1 block text-xs font-medium text-foreground">{t("password")}</label>
              <div className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2.5">
                <Lock className="h-4 w-4 text-muted-foreground" />
                <input type={showPassword ? "text" : "password"} placeholder={t("enter_password")} className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
            ...
            {mode === "login" && (
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 text-xs text-muted-foreground">
                  <input type="checkbox" className="accent-primary" /> {t("remember_me")}
                </label>
                <button type="button" className="text-xs text-primary hover:underline">{t("forgot_password")}</button>
              </div>
            )}
            ...
            <button type="submit" className="w-full rounded-lg bg-primary py-3 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90">
              {mode === "login" ? t("sign_in") : t("create_account_btn")}
            </button>
          </form>
